...
def generate_budget(scen_data: pd.DataFrame, cost_data: pd.DataFrame, settings: CostSettingItems, target_population: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    if 'code_itn_campaign' in scen_data.columns and scen_data['code_itn_campaign'].sum() > 0:
        itn_q = scen_data[scen_data['code_itn_campaign'] == 1].merge(
            ...
            target_population, left_on=['adm1', 'adm2'], right_on=['adm1', 'adm2'], suffixes=('', '_right') 
        )
        itn_q['target_pop'] = itn_q['pop_total'] * settings.itn_campaign_coverage
        itn_q['quant_nets'] = (itn_q['target_pop'] / settings.itn_campaign_divisor) * settings.itn_campaign_buffer
        itn_q['quant_bales'] = itn_q['quant_nets'] / settings.itn_campaign_bale_size
        itn_q_long = itn_q.melt(
            id_vars=['adm1', 'adm2', 'year', 'scenario_name', 'scenario_description', 'target_pop', 'type_itn_campaign'],
            value_vars=['quant_nets', 'quant_bales'], var_name='unit', value_name='quantity'
        ).rename(columns={'type_itn_campaign': 'type_intervention'})
        itn_q_long['unit'] = itn_q_long['unit'].replace({'quant_nets': 'per ITN', 'quant_bales': 'per bale'})
        itn_q_long['code_intervention'] = 'itn_campaign'
    else:
        itn_q_long = pd.DataFrame()

    #--- ITN ROUTINE ---
    if 'code_itn_routine' in scen_data.columns and scen_data['code_itn_routine'].sum() > 0:
        ...
        itn_r_q['target_pop'] = (itn_r_q['pop_0_5'] + itn_r_q['pop_pw']) * settings.itn_routine_coverage
        itn_r_q['quant_nets'] = itn_r_q['target_pop'] * settings.itn_routine_buffer
        itn_r_q_long = itn_r_q.melt(
            id_vars=['adm1', 'adm2', 'year', 'scenario_name', 'scenario_description', 'target_pop', 'type_itn_routine'],
            value_vars=['quant_nets'], var_name='unit', value_name='quantity'
    ...
    if 'code_iptp' in scen_data.columns and scen_data['code_iptp'].sum() > 0:
        ...
        iptp_q['quant_sp_doses'] = (iptp_q['target_pop'] * settings.iptp_doses_per_pw) * settings.iptp_buffer
        iptp_q_long = iptp_q.melt(
            id_vars=['adm1', 'adm2', 'year', 'scenario_name', 'scenario_description', 'target_pop', 'type_iptp'],
            value_vars=['quant_sp_doses'], var_name='unit', value_name='quantity'
    ...
    if 'code_smc' in scen_data.columns and scen_data['code_smc'].sum() > 0:
        ...
        smc_q['quant_smc_spaq_3_11_months'] = (smc_q['pop_0_5'] * smc_pop_prop_3_11 * smc_coverage) * smc_monthly_rounds * smc_buffer
        smc_q['quant_smc_spaq_12_59_months'] = (smc_q['pop_0_5'] * smc_pop_prop_12_59 * smc_coverage) * smc_monthly_rounds * smc_buffer
        smc_q['quant_smc_spaq_5_10_years'] = (smc_q['pop_5_10'] * smc_coverage * smc_monthly_rounds * smc_buffer) if include_5_10 and 'pop_5_10' in smc_q.columns else 0
        
        pop_base = smc_q['pop_0_5'] * (smc_pop_prop_3_11 + smc_pop_prop_12_59)
        pop_extended = (pop_base + smc_q['pop_5_10']) if include_5_10 and 'pop_5_10' in smc_q.columns else pop_base
        smc_q['target_pop'] = pop_extended * smc_coverage
        
        smc_q_long = smc_q.melt(
            id_vars=['adm1', 'adm2', 'year', 'scenario_name', 'scenario_description', 'target_pop', 'type_smc'],
            value_vars=[c for c in smc_q.columns if c.startswith('quant_smc')],
            var_name='unit', value_name='quantity'
    ...
    if not pmc_df.empty:
        ...
        target_pop_pmc = target_population.rename(columns={'annee': 'year'})[pop_cols_to_merge]
        ...
        pmc_df = pd.merge(pmc_df, target_pop_pmc, on=['adm1', 'adm2'], how='left', suffixes=('', '_right'))
        ...
        pmc_df['quant_pmc_child'] = (
            pmc_df['pop_0_1'] * settings.pmc_coverage + pmc_df['pop_1_2'] * settings.pmc_coverage
        )
        ...
        pmc_quantification = pd.melt(
            ...
            id_vars=[
                ...
                'code_intervention', 'type_intervention', 'target_pop'
            ],
        ...
        pmc_quantification['unit'] = pmc_quantification['unit'].str.replace('quant_pmc_', '')
        ...
        pmc_quantification = pd.DataFrame() # Create empty dataframe if no PMC intervention
    ...
    if 'code_vacc' in scen_data.columns and scen_data['code_vacc'].sum() > 0:
        ...
        vacc_q['quant_child'] = vacc_q['pop_vaccine_5_36_months'] * settings.vacc_coverage
        vacc_q['quant_doses'] = vacc_q['quant_child'] * settings.vacc_doses_per_child * settings.vacc_wastage_offset
        vacc_q_long = vacc_q.melt(
            id_vars=['adm1', 'adm2', 'year', 'scenario_name', 'scenario_description', 'type_vacc'],
            value_vars=['quant_doses', 'quant_child'], var_name='unit', value_name='quantity'
        ).rename(columns={'type_vacc': 'type_intervention'})
        vacc_q_long['code_intervention'] = 'vacc'
        vacc_q_long = vacc_q_long.assign(unit=lambda df: df["unit"].replace({"quant_doses": "per dose", "quant_child": "per child"}))
    else:
        vacc_q_long = pd.DataFrame()

    #--- CASE MANAGEMENT ---
    try:
        case_management_quantification = pd.read_csv("./nga-demo-data-pre-processed/cm-quant-data.csv")
        case_management_quantification = case_management_quantification.assign(
            ...
            year=scen_data["year"].iloc[0], # Assumes a single year in scen_data for this part
            code_intervention="cm_public",
        )
        case_management_quantification = case_management_quantification.melt(
            id_vars=["adm1", "adm2", "year", "scenario_name", "scenario_description", "code_intervention"],
            value_vars=[col for col in case_management_quantification.columns if col.startswith("cm")],
            var_name="unit",
            value_name="quantity",
        )
        ...
        case_management_quantification["type_intervention"] = np.select(type_intervention_conditions, type_intervention_choices, default="Other")
        ...
        case_management_quantification["unit"] = np.select(unit_conditions, unit_choices, default="Other")
    ...
    # Join with cost data
    budget = pd.merge(budget, cost_data, on=["code_intervention", "type_intervention", "unit"], how="left")
    ...
    budget = budget.assign(
        ...
        currency=lambda df: df["currency"].replace({"usd_cost": "USD", "ngn_cost": "NGN"}),
    )
    ...
    budget["intervention_nice"] = np.select(conditions, choices, default=budget["code_intervention"])
...
def get_budget(data, country, year, settings):
   ...
   try:
      ...
      scen_data["adm0"] = country  # Add a new column 'adm0' and set its value to "Nigeria"
      ...
      def set_intervention_code(intervention_name, column_name):
         ...
         ########################################################################
         intervention = [intervention for intervention in data.interventions if intervention.name == intervention_name]
         intervention_places = intervention[0].places if len(intervention) > 0 else []
         scen_data[column_name] = scen_data.apply(
            lambda row: 1 if (f'{row["adm1"]}:{row["adm2"]}' in intervention_places) else 0,
            axis=1
         )
      def set_intervention_type(intervention_name, column_name):
         ...
         ################################################################
         intervention = [intervention for intervention in data.interventions if intervention.name == intervention_name]
         scen_data[column_name] = intervention[0].type if len(intervention) > 0 else ""
      ...
      if (input_costs_dict.__len__() > 0):
        ...
        validation = cost_data.merge(pd.DataFrame(input_costs_dict), on=["code_intervention", "type_intervention", "cost_class", "unit"],
          how="inner", suffixes=("", "_y"))

        if (validation.__len__() != input_costs_dict.__len__()):
          raise ValueError("Cost data override validation failed.")

        cost_data = cost_data.merge(pd.DataFrame(input_costs_dict), on=["code_intervention", "type_intervention", "cost_class", "unit"],
          how="left", suffixes=("", "_y"))
        cost_data["usd_cost"] = cost_data["usd_cost_y"].combine_first(cost_data["usd_cost"])

        # TODO: need a better way to support different country
        if (country == "NGA"):  
          cost_data["ngn_cost"] = cost_data["ngn_cost_y"].combine_first(cost_data["ngn_cost"])
        elif (country == "CDF"):  # DRC
          cost_data["cdf_cost"] = cost_data["cdf_cost_y"].combine_first(cost_data["cdf_cost"])
      ...
      def get_cost_class_data(code, currency, year, cost_class):
          ...
          """
          cost = budget[(budget["code_intervention"] == code) & (budget["currency"] == currency.upper()) &
                (budget["year"] == year) & (budget["cost_class"] == cost_class)]["cost_element"].sum()
          pop = budget[(budget["code_intervention"] == code) & (budget["currency"] == currency.upper()) &
                (budget["year"] == year) & (budget["cost_class"] == cost_class)]["target_pop"].sum()
      ...
      interventions = [
        ...
        "iptp", "smc", "pmc", "vacc", "itn_routine", "itn_campaign"
      ]
      ...
      for code, name in zip(interventions, interventions):
        ...
        for cost_class in cost_classes:
            cost_class_data = get_cost_class_data(code, settings.currency, year, cost_class)
            if cost_class_data["cost"] > 0:
                costs.append({"name": name, "cost_class": cost_class, "cost": cost_class_data["cost"]})
            total_cost += cost_class_data["cost"]
            total_pop += cost_class_data["pop"]
        intervention_costs["interventions"].append({"name": name, "total_cost": total_cost,
                                                    "total_pop": total_pop, "cost_breakdown": costs})
