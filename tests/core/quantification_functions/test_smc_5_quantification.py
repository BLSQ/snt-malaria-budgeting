...
class TestSMC5Quantification(unittest.TestCase):
    def setUp(self):
        ...
        self.scen_data = pd.DataFrame(
            {
                ...
                "adm2": [admin2],
                "year": [year],
                "scenario_name": ["Scenario with SMC_5 intervention code"],
                "scenario_description": ["Test with only SMC_5 intervention code"],
                "code_smc_5": [1],
                "type_smc_5": ["SMC_5"],
            }
        )
    ...
    def test_smc_5_quantification(self):
        """Test that SMC5Quantification returns expected results for a known code."""

        quant = SMC5Quantification(
            spatial_unit="adm2",
            assumptions={
                "smc_5_coverage": 0.5,
                "smc_5_buffer_mult": 1.1,
                "smc_5_monthly_rounds": 1.5,
                "smc_5_pop_prop_3_11": 0.6,
                "smc_5_pop_prop_12_59": 0.3,
            },
        )
    ...
    def test_smc_5_quantification_custom_target_population(self):
        """Test that SMC5Quantification returns expected results for a known code."""
        ...
        scen_data["target_population_columns_smc_5"] = [["pop_0_5_custom"]]
    ...
    def test_smc_5_quantification_multiple_custom_target_population(self):
        """Test that SMC5Quantification returns expected results for a known code."""
        ...
        scen_data = self.scen_data.copy()
        scen_data["target_population_columns_smc_5"] = [
    ...
    def test_smc_5_quantification_empty_base(self):
        """Test that SMC5Quantification returns an empty DataFrame when base data is empty."""
    ...
    def test_smc_5_quantification_missing_assumption(self):
        """Test that SMC5Quantification uses default coverage when specific coverage assumption is missing."""

        quant_missing_coverage = SMC5Quantification(
            spatial_unit="adm2",
            assumptions={
                "smc_5_buffer_mult": 1.1,
                "smc_5_monthly_rounds": 1.5,
        ...
        with self.assertRaisesRegex(
            ValueError, r"Missing assumptions for smc_5: smc_5_coverage"
        ):
            quant_missing_coverage.get_quantification(
                self.scen_data, self.mock_population_data
            )

        quant_missing_monthly_rounds = SMC5Quantification(
            spatial_unit="adm2",
            assumptions={
                "smc_5_coverage": 0.5,
                "smc_5_buffer_mult": 1.1,
        ...
        with self.assertRaisesRegex(
            ValueError, r"Missing assumptions for smc_5: smc_5_monthly_rounds"
        ):
            quant_missing_monthly_rounds.get_quantification(
                self.scen_data, self.mock_population_data
            )

        quant_missing_buffer_mult = SMC5Quantification(
            spatial_unit="adm2",
            assumptions={
                "smc_5_coverage": 0.5,
                "smc_5_monthly_rounds": 1.5,
        ...
        with self.assertRaisesRegex(
            ValueError, r"Missing assumptions for smc_5: smc_5_buffer_mult"
        ):
            quant_missing_buffer_mult.get_quantification(
                self.scen_data, self.mock_population_data
            )

        quant_missing_prop_3_11 = SMC5Quantification(
            spatial_unit="adm2",
            assumptions={
                "smc_5_coverage": 0.5,
                "smc_5_buffer_mult": 1.1,
        ...
        with self.assertRaisesRegex(
            ValueError, r"Missing assumptions for smc_5: smc_5_pop_prop_3_11"
        ):
            quant_missing_prop_3_11.get_quantification(
                self.scen_data, self.mock_population_data
            )

        quant_missing_prop_12_59 = SMC5Quantification(
            spatial_unit="adm2",
            assumptions={
                "smc_5_coverage": 0.5,
                "smc_5_buffer_mult": 1.1,
        ...
        with self.assertRaisesRegex(
            ValueError, r"Missing assumptions for smc_5: smc_5_pop_prop_12_59"
        ):
            quant_missing_prop_12_59.get_quantification(
                self.scen_data, self.mock_population_data
            )

    def test_smc_5_quantification_different_code(self):
        """Test that SMC5Quantification returns empty DataFrame when code does not match any column in scen_data."""
    ...
    def test_smc_5_quantification_missing_pop_column(self):
        """Test that SMC5Quantification raises an error when population column is missing."""
